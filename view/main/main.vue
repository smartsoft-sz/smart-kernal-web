...
<template>
  <Layout style="height: 100%" class="main">
    <Sider hide-trigger collapsible :width="220" :collapsed-width="64" v-model="collapsed" class="left-sider" :style="{overflow: 'hidden'}">
      <side-menu v-if="menuLoaded" accordion ref="sideMenu" :active-name="$route.name" :collapsed="collapsed" @on-select="turnToPage" :menu-list="menuList">
...
import { getNewTagList, getNextName, getMenuByAccess } from '@/libs/util'
...
//      let userAccess = this.$store.state.user.access;
      let permissionNames = this.$store.state.user.permissionNames;
      menuList.forEach(item => {
        if (!item.meta) item.meta = {};
        let found = tableMenuList.find(menu => menu.routerName === item.name);
        let menuPermissions = found && found.permissionList.map(item => item.name);
        if (menuPermissions && hasOneOf(menuPermissions, permissionNames)) {
          if (found) {
...
//      let userAccess = this.$store.state.user.access
      let permissionNames = this.$store.state.user.permissionNames;
      let menuList = getMenuByAccess(routers, permissionNames)
      let tableMenuList = this.$store.state.app.tableMenuList;
      if (tableMenuList && tableMenuList.length > 0) {
        this.fuseMenuList(menuList, tableMenuList)
        this.setMenuList(getMenuByAccess(menuList, permissionNames))
        this.menuLoaded = true
      } else {
        this.getTableMenuList().then(tableMenuList => {
          this.fuseMenuList(menuList, tableMenuList)
          this.setMenuList(getMenuByAccess(menuList, permissionNames))
          this.menuLoaded = true
        })
