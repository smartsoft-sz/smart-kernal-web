<template>
  <div>
    <textarea v-bind:id="id">{{ content }}</textarea>
  </div>
</template>
...
type: Array
      ...
      toolbar1: { default: 'fullscreen |  bold italic  strikethrough  forecolor backcolor | table link image | alignleft aligncenter alignright alignjustify  | numlist bullist indent  | removeformat', type: String },
...
//            };
            vm.$http.post('/common/file/upload', formData).then(resp => {
              if (resp.data.code === 200) {
                success(resp.data.data.location);
              } else {
