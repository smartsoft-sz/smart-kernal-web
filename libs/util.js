...
export const setPermissionNames = permissionNames => {
  Cookies.set(PERMISSION_KEY, permissionNames.join(','))
};
export const getPermissionNames = () => {
  const permissionNames = Cookies.get(PERMISSION_KEY);
  return permissionNames && permissionNames.split(',')
};
export const clearPermissionNames = () => {
 ...
 * @returns {Array}
 */
export const getMenuByAccess = (list, access) => {
  let res = []
  forEach(list, item => {
    if (!item.meta || (item.meta && !item.meta.hideInMenu)) {
      ...
      if ((hasChild(item) || (item.meta && item.meta.showAlways)) && showThisMenuEle(item, access)) {
        obj.children = getMenuByAccess(item.children, access)
